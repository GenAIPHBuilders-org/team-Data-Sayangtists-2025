    ...
    for lineno, line in enumerate(file, start=1):
        ...
        if kind in ABIItem.KINDS:
            ...
            if prev := getattr(parent, "struct_abi_kind", None):
                raise_error(f"{parent.name} already has {prev}, cannot add {kind}")
...
def gen_python3dll(manifest, args, outfile):
    ...
    for item in sorted(
        manifest.select({"function"}, include_abi_only=True, ifdef={"MS_WINDOWS"}),
        key=sort_key,
    ):
    ...
    for item in sorted(
        manifest.select({"data"}, include_abi_only=True, ifdef={"MS_WINDOWS"}),
        key=sort_key,
    ):
