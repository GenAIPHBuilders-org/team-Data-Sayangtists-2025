...
def _prepare_paramspec_params(cls, params):
    ...
    if len(cls.__parameters__) == 1 and params and not _is_param_expr(params[0]):
...
def Concatenate(self, parameters):
    ...
    return _ConcatenateGenericAlias(
        self, parameters, _typevar_types=(TypeVar, ParamSpec), _paramspec_tvars=True
    )
...
class _GenericAlias(_BaseGenericAlias, _root=True):
    ...
    def __getitem__(self, params):
        ...
        if self._paramspec_tvars and any(
            isinstance(t, ParamSpec) for t in self.__parameters__
        ):
    ...
    def __reduce__(self):
        ...
        if len(args) == 1 and (
            not isinstance(args[0], tuple) or origin is Tuple and not args[0]
        ):
            (args,) = args
...
def get_args(tp):
    ...
    if isinstance(tp, (_GenericAlias, GenericAlias)):
        res = tp.__args__
        if tp.__origin__ is collections.abc.Callable and not (
            len(res) == 2 and _is_param_expr(res[0])
        ):
